...
class PlannerNode : public rclcpp::Node
{
  ...
  private:
     ...
     * checks on paths should always be measured to when it was created or else paths could be sent later
     * and TTL checks would pass on old paths -- i.e., not good.
     *
     ...
     * Speed profiles are lists of speeds that control should follow. They map to path nodes and apply to all subsequent nodes.
     *
     * For example,
     * A speed profile of { 1.0 } applies a 1m/s speed target to all waypoints in the target path.
     * A speed profile of { 1.0, 2.0 } applies a 1m/s speed target to the first waypoint and 2m/s target to all following waypoints.
     * 
